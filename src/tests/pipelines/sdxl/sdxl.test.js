...
const {InferenceSession} = ORT.default;
/**
 * @param {string} tokenizerPath 
 */
function createTokenizer (tokenizerPath) {
  const merges          =            readFileSync(`${tokenizerPath}/merges.txt`             , 'utf-8');
  const vocab           = JSON.parse(readFileSync(`${tokenizerPath}/vocab.json`             , 'utf-8'));
  const tokens          = JSON.parse(readFileSync(`${tokenizerPath}/special_tokens_map.json`, 'utf-8'));
  const tokenizerConfig = JSON.parse(readFileSync(`${tokenizerPath}/tokenizer_config.json`  , 'utf-8'));
  const tokenizerJSON = {
    normalizer: {
      type: 'Lowercase',
    },
...
describe('SDXL', () => {
  it ('should encode prompt', async () => {
    ...
    const schedulerConfig = JSON.parse(readFileSync(`${modelDir}/scheduler/scheduler_config.json`, 'utf-8'));
    ...
    const expectedPrompt = JSON.parse(readFileSync('src/tests/pipelines/sdxl/prompt_embeds.json', 'utf-8'));
    ...
    const expectedTextEmbeds = JSON.parse(readFileSync('src/tests/pipelines/sdxl/text_embeds.json', 'utf-8'));
